...
def dag_bigquery_to_huggingface():
    ...
    def bigquery_to_gcs(
        ...
        prefix = f"{gcs_object_prefix}{temp_name}_"
        destination_uri = f"gs://{gcs_bucket_name}/{prefix}*.csv"
    ...
    def register_dataset_to_huggingface(
        ...
        gcs_hook = GCSHook()
        blobs = gcs_hook.list(gcs_bucket_name, prefix=prefix)

        dfs = []
        for blob_name in blobs:
            blob_content = gcs_hook.download(gcs_bucket_name, blob_name)
            df = pd.read_csv(io.BytesIO(blob_content), sep=",")
            dfs.append(df)
        merged_df = pd.concat(dfs, ignore_index=True)

        with io.BytesIO() as byte_stream:
            merged_df.to_csv(byte_stream, index=False)
            byte_stream.seek(0)
            api.upload_file(
    ...
    gcs_bucket_name = "airflow_jh"
    gcs_object_prefix = "wikipedia_dataset_"

    prefix = bigquery_to_gcs(
    ...
    register_dataset_to_huggingface(
        gcs_bucket_name=gcs_bucket_name,
        prefix=prefix,
        dataset_repo_name="jh_wikipedia",
    )
